...
import {requestOptions_ttxc, requestOptions_s168} from './Helper'
import  gsap from 'gsap'
function S168Scene_num() {
	const { id } = useParams();
	...
	const onSceneReady = (scene,engine) => {
		...
		const fetchDataList = async ()=>{
			...
			const ids = await (await fetch(url, requestOptions_s168)).json()
				console.log(ids)
				const data = Promise.all(
					ids.map(async (i) => await (await fetch(`https://fullbodyscan-dev.msvfx.com/api/guest_fbs_photo_profile/${i.id}`,requestOptions_s168 )).json())
				)
				return data 
		}
		...
		const getFirstData = async(id)=>{
			const data =  await (await fetch(`https://fullbodyscan-dev.msvfx.com/api/guest_fbs_photo_profile/${id}`,requestOptions_s168 )).json()
			console.log(data)
